...
1. Modify the DBMS configuration to suit your needs, the provided example contains the most minimal configuration required to run the Operator, you can find it under the file `config.example.yaml` or in the Helm chart under `operatorConfig`.
    ...
    4. Under `operations`, describe the `create`, `delete` and `rotate` stored procedures.
        1. For each operation:
            1. `name` is a string specifying the name of the stored procedure as is in your DBMS.
            2. `inputs` is a `map[k]v` input parameters for the stored procedure where key is the `name` of the parameter.
